...
public class formatSwitch {
    ...
    public formatSwitch(Context context, ImageButton buttonSort){
    ...
    protected int getSettingsFormatParam(){
        return PreferenceManager
                .getDefaultSharedPreferences(context).getInt("formatParam", SystemCostant.Setting_Format);
    }
    ...
    public void formatNote(){
     switch (getSettingsFormatParam()){
         case 1:
            context.getSharedPreferences(settingsFileName, Context.MODE_PRIVATE).edit()
                     .putInt("formatParam", 2).apply();
             buttonSort.setImageResource(getParamIco(2));
             break;
         case 2:
             context.getSharedPreferences(settingsFileName, Context.MODE_PRIVATE).edit()
                     .putInt("formatParam", 1).apply();
