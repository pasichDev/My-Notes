...
public  class FragmentAppInfo extends PreferenceFragmentCompat {
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        assert ratings != null;
        ratings.setOnPreferenceClickListener(preference -> {
            ...
            { startActivity(rateAppIntent); }else{
                Toast.makeText(getContext(),
                        getString(R.string.notFoundPlayMarket),
                        Toast.LENGTH_SHORT).show();
            }
            return true;
        });
        assert whatsUpdateKey != null;
        whatsUpdateKey.setOnPreferenceClickListener(preference -> {
            new AlertDialog.Builder(getActivity())
                    .setTitle(versionName)
                    .setMessage(getString(R.string.updateNowM))
