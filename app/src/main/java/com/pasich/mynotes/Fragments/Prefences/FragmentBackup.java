    ...
    implements RestoreNotes.continueImport
{
     ...
     */
    ActivityResultLauncher<Intent> startIntentImport = registerForActivityResult(
            ...
            result -> {
                if (result.getResultCode() == Activity.RESULT_OK) {
                    ...
                    if (data != null) {
                        ...
                        try {
                            ParcelFileDescriptor descriptor = getContext().getApplicationContext().getContentResolver()
     ...
     */
    ActivityResultLauncher<Intent> startIntentExport = registerForActivityResult(
            new ActivityResultContracts.StartActivityForResult(),
            result -> {
                if (result.getResultCode() == Activity.RESULT_OK) {
                    Intent data = result.getData();
                    if (data != null) {
                        try {
                            ParcelFileDescriptor descriptor = getContext().getContentResolver()
                                    .openFileDescriptor(data.getData(), "w");
                            FileOutputStream outputStream = new FileOutputStream(descriptor.getFileDescriptor());
                            backupClass.copyFile(new File(getContext().getFilesDir()+"/"+getNameBackup()), outputStream);
                            Toast.makeText(getContext(),
                                    getString(R.string.backupIsCreate),
                                    Toast.LENGTH_SHORT).show();
                        } catch (FileNotFoundException e) {
                            Toast.makeText(getContext(),
